...
public sealed class SmsService
{
    ...
    public async Task<SmsResult> SendAsync(string toPhone, string message)
    {
        if (!IsConfigured)
        {
            _log.LogWarning("[SMS] Twilio not configured. Would send to {Phone}: {Message}", toPhone, message);
        ...
        try
        {
            ...
            {
                _log.LogInformation("[SMS] Sent to {Phone}", toPhone);
                return new SmsResult { Success = true, MessageId = body };
            }

            _log.LogError("[SMS] Failed to {Phone}: {Status} {Body}", toPhone, resp.StatusCode, body);
            return new SmsResult { Success = false, Error = body };
        }
        catch (Exception ex)
        {
            _log.LogError(ex, "[SMS] Exception sending to {Phone}", toPhone);
