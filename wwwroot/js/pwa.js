...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    if (isLocalDevHost) {
      // In local development, stale SW cache often causes CSS/JS mismatch with production.
...
function showUpdateBanner() {
  ...
  banner.style.cssText =
    "position:fixed;bottom:0;left:0;right:0;background:#1e3a5f;color:#fff;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;z-index:9999;font-size:14px;";
  banner.innerHTML =
    '<span>A new version of RetailERP is available.</span><button onclick="location.reload()" style="background:#4fc3f7;color:#1e3a5f;border:none;padding:6px 16px;border-radius:4px;font-weight:600;cursor:pointer;">Update Now</button>';
...
const NetworkStatus = {
  ...
  _createIndicator() {
    ...
    el.style.cssText =
      "position:fixed;top:0;left:0;right:0;z-index:10000;text-align:center;font-size:13px;font-weight:600;padding:4px 0;transition:transform 0.3s;transform:translateY(-100%);";
  ...
  _setOffline() {
    ...
    el.style.color = "#fff";
    el.textContent =
      "⚠ No Internet — POS bills will be saved offline and synced when reconnected";
    el.style.transform = "translateY(0)";
  },
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = (e) => {
        ...
        if (!db.objectStoreNames.contains("offlineBills")) {
          const store = db.createObjectStore("offlineBills", {
            keyPath: "localId",
          });
    ...
    try {
      ...
      for (const bill of pendingBills) {
        try {
          ...
          if (result.success) {
            await OfflineDB.updateBillStatus(
              bill.localId,
              "synced",
              result.syncLogId,
            );
